...
public class BSTCipher {
	...
	public static NodeCipher bstEnc(Node node, int beta) {
		...
		if(node.isLeaf == true) {
			...
			ArrayList<Shares> xDatasetShares = TPSS.share(leafNode.xDataset);
			ArrayList<Shares> wDatasetShares = TPSS.share(leafNode.wDataset);
			...
			Shares bShares_node = TPSS.share(1);
			...
			Shares wShares_A_node = TPSS.share(w_A_node);
	...
	public static Shares[] identitylv(Shares bShares, Shares[] lShares, Shares[] vShares, Key key) throws Exception {
		...
		for(int i = 0; i < lShares.length; i++) {
			...
			Shares tmp1 = TPSS.ASE_mul(bShares, lShares[i], key);
			Shares tmp2 = TPSS.ASE_sub(1, bShares);
			tmp2 = TPSS.ASE_mul(tmp2, vShares[i], key);
	...
	public static CandidateResult bstSearchCipher(NodeCipher treeCipher, ArrayList<Shares[]> queryShares, Key key) throws Exception {
		...
		int commSizeOfASEMul = 192; // 3 64-bits shares
		int commSizeOfASERecover = 64; // 1 64-bits shares
		int numOfASEMul = 0;
...
//					System.out.println("internal node x = " + ASE.recoverBinary(lShares_node) + ";" + ASE.recoverBinary(uShares_node));
	...
	public static Shares resultVerify(CandidateResult candidateResult, int delta, int eta, Key key) throws Exception {
		...
		int commSizeOfASEMul = 192; // 3 64-bits shares
		int commSizeOfASERecover = 64; // 1 64-bits shares
		int commSizeOfShare = 64; // 64-bits shares
		...
		for(int i = 1; i < wShares.size(); i++) {
			Shares tmp = TPSS.ASE_add(wSumShares.get(i - 1), wShares.get(i));
			wSumShares.add(tmp);
		}
		
		
//		System.out.println("delta = " + delta);
		// 2*delta*ASE_share + delta*ASE_mul
		Shares rShares = PrivateAlgs.PURDG(delta, key);
		numOfShare = numOfShare + 8*key.delta;
...
//		System.out.println("rShares = " + ASE.recover(rShares));
		...
		Shares rMulwSumShares = TPSS.ASE_mul(rShares, wSumShares.get(wSumShares.size() - 1), key);
		numOfASEMul = numOfASEMul + 1;
		
		for(int i = 0; i < wSumShares.size(); i++) {
			Shares tmp = TPSS.ASE_scalP(wSumShares.get(i), BigInteger.TWO.pow(delta));
			Shares zShares = TPSS.ASE_sub(rMulwSumShares, tmp);
...
//			System.out.println("i = " + i + ";wSumShares = " + ASE.recover(wSumShares.get(i)));
			boolean flag = PrivateAlgs.PADC(zShares, eta); // equivalent to 5/6 ASE_share
...
//		System.out.println("loc = " + loc);
		...
		if (cWShares.size() == 0) {
			...
			// Comm: (5*n + 8*delta + 5)*ASE_share + (4*n + delta + 4)*ASE_mul
			numOfShare = numOfShare + 5*aliasCipher.length + 8*key.delta + 5;
