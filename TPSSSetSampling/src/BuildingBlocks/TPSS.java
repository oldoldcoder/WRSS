...
public class TPSS {
	...
	public static TPSS ase = new TPSS();
	...
	public static Shares ASE_add(Shares xShares, int y) {
		...
		s1 = xShares.s1.add(BigInteger.valueOf(y)).mod(modVal);
	...
	public static Shares ASE_sub(int x, Shares yShares) {
		...
		s1 = BigInteger.valueOf(x).subtract(yShares.s1).mod(modVal);
		
		Shares zShares = TPSS.ase.new Shares(s1, yShares.s2.negate(), yShares.s3.negate());
		return zShares;
...
//	public static BigInteger SHA256(String str) throws Exception {
//		
//		MessageDigest digest = MessageDigest.getInstance("SHA-256");
//    	byte[] encodedhash = digest.digest(str.getBytes(StandardCharsets.UTF_8));
//    	BigInteger cipherBN = new BigInteger(encodedhash);
//    	cipherBN = cipherBN.mod(modVal);
//    	return cipherBN;	
//	}
	...
	public static Shares ASE_mul(Shares xShares, Shares yShares, Key key) throws Exception {
		...
		for(int i = 0; i < 3; i++) {
			r[i] = TPSS.hmacSHA256(key.pair_key.get(i), sidByte1);
			r[i] = TPSS.hmacSHA256(key.pair_key.get(i), sidByte1);
		}
		...
		Shares zShares = TPSS.ase.new Shares(s1Ply, s2Ply, s3Ply);
		return zShares;
