...
public class PrivateAlgs {
	...
	public static VecShares PIVG(int numRecords, Key key) throws Exception {
		...
		VecShares vVecShares = TPSS.share(v);
		
		// generate a random number r
		BigInteger r = Util.genRandnum(key.pair_key.get(1));
		r = Util.genRandnum(key.pair_key.get(1));
		
		// generate a random vector
		BigInteger[] r2 = Util.genRandnumVec(numRecords, key.pair_key.get(1));
		r2 = Util.genRandnumVec(numRecords, key.pair_key.get(1));
		BigInteger[] r3 = Util.genRandnumVec(numRecords, key.pair_key.get(1));
		r3 = Util.genRandnumVec(numRecords, key.pair_key.get(1));
		
		// CS3 computes r1
		BigInteger[] r1 = Util.bigIntegerAdd(r2, r3, TPSS.modVal);
		r1 = Util.negativeBigVec(r1, TPSS.modVal);
		
		BigInteger[] v1plyply = Util.bigIntegerAdd(vVecShares.s1Vec, r1, TPSS.modVal);
		BigInteger[] v2plyply = Util.bigIntegerAdd(vVecShares.s2Vec, r2, TPSS.modVal);
		BigInteger[] v3plyply = Util.bigIntegerAdd(vVecShares.s3Vec, r3, TPSS.modVal);
	...
	public static Shares PURDG(int delta, Key key) throws Exception {
		...
		for(int i = 0; i < bitLength; i++) {
			...
			r2Shares[i] = TPSS.share(r2[i]);
		}
		
		Shares tmp1 = TPSS.ASE_add(r1Shares[0], r2Shares[0]); 
		Shares tmp2 = TPSS.ASE_mul(r1Shares[0], r2Shares[0], key);
		tmp2 = TPSS.ASE_scalP(tmp2, -2);
		tmp2 = TPSS.ASE_add(tmp1, tmp2);
		Shares z = TPSS.ASE_scalP(tmp2, (int) Math.pow(2, 0));
		
		for(int i = 1; i < bitLength; i++) {
			
			tmp1 = TPSS.ASE_add(r1Shares[i], r2Shares[i]); 
			tmp2 = TPSS.ASE_mul(r1Shares[i], r2Shares[i], key);
			tmp2 = TPSS.ASE_scalP(tmp2, -2);
			tmp2 = TPSS.ASE_add(tmp1, tmp2);
			tmp2 = TPSS.ASE_scalP(tmp2, (int) Math.pow(2, i));
			z = TPSS.ASE_add(z, tmp2);
		}
	...
	public static Shares PRDG(Key key) {
		...
		BigInteger r1 = new BigInteger(TPSS.n, rnd);
		r1 = r1.mod(TPSS.modVal);
		BigInteger r2 = new BigInteger(TPSS.n, rnd);
		r2 = r2.mod(TPSS.modVal);
		BigInteger r3 = new BigInteger(TPSS.n, rnd);
		r3 = r3.mod(TPSS.modVal);
		
		Shares z = TPSS.ase.new Shares(r1, r2, r3);
		return z;
	}
	...
	public static boolean PBDC(Shares[] x, Shares[] y, Key key) throws Exception {
		...
		for(int i = 0; i < beta; i++) {
			Shares tmp1 = TPSS.ASE_add(x[i], y[i]);
			Shares tmp2 = TPSS.ASE_mul(x[i], y[i], key);
			tmp2 = TPSS.ASE_scalP(tmp2, -2);
			w[i] = TPSS.ASE_add(tmp1, tmp2);
		}
...
//		System.out.println();
		...
		Shares tmp1 = TPSS.ASE_sub(x[0], y[0]);
		tmp1 = TPSS.ASE_add(tmp1, 1);
		Shares productC = TPSS.ASE_add(tmp1, sumW[0]);
		
		
		// beta*ASE_mul
		for(int i = 0; i < beta; i++) {
			tmp1 = TPSS.ASE_sub(x[i], y[i]);
			tmp1 = TPSS.ASE_add(tmp1, 1);
			if(i != beta - 1) {
				tmp1 = TPSS.ASE_add(tmp1, sumW[i]);
			}
			productC = TPSS.ASE_mul(tmp1, productC, key);
		}
		
		Shares rShares = PRDG(key);
		rShares = PRDG(key);
		
		rShares = TPSS.ASE_mul(rShares, productC, key);
		
		BigInteger r = TPSS.recover(rShares);
...
//		System.out.println();
	...
	public static boolean PRCT(Shares[] l, Shares[] u, Shares[] l_node, Shares[] u_node, Key key) throws Exception {
		...
		Shares rShares = PRDG(key);
		rShares = TPSS.ASE_mul(rShares, c1, key);
		rShares = TPSS.ASE_mul(rShares, c2, key);
...
//		System.out.println("alpha:" + alpha);
	...
	public static Shares[] PADC_aliasSampling(Shares xShares, int eta) {
		...
		Shares gamma1Share = TPSS.ase.new Shares(BigInteger.valueOf(gamma1), BigInteger.ZERO, BigInteger.ZERO);
		Shares gamma3Share = TPSS.ase.new Shares(BigInteger.ZERO, BigInteger.ZERO, BigInteger.valueOf(gamma3));
	...
	public static Shares reShare(Shares x, int eta) {
		...
		BigInteger x3 = x.s3.add(x1miusu1).add(x2miusu2).mod(TPSS.modVal);
		x3 = x3.subtract(TPSS.modVal);
		
		Shares newX = TPSS.ase.new Shares(u1, u2, x3);
	...
	public static Shares[] ToBinAndEnc(int x, int beta) {
		...
		Shares[] xBinaryShares = TPSS.shareVec(xBinary);
